...
def set_smtlib2(rounds):
    ...
    for r in range(rounds):
        ...
        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_5_{r}_1 x_6_{r}_0) z_6_{r}_0)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_4_{r}_1 x_7_{r}_0) z_7_{r}_0)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_3_{r}_1 x_8_{r}_0) z_8_{r}_0)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_2_{r}_1 x_9_{r}_0) z_9_{r}_0)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_1_{r}_1 x_10_{r}_0) z_10_{r}_0)) #b1))\n")

        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_0_{r}_1 x_11_{r}_0) z_11_{r}_0)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_1_{r}_0 z_11_{r}_0) z_11_{r}_1)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_2_{r}_0 z_11_{r}_1) z_11_{r}_2)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_3_{r}_0 z_11_{r}_2) z_11_{r}_3)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_4_{r}_0 z_11_{r}_3) z_11_{r}_4)) #b1))\n")

        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_5_{r}_0 z_11_{r}_4) z_11_{r}_5)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_5_{r}_0 z_10_{r}_0) z_10_{r}_1)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_5_{r}_0 z_9_{r}_0) z_9_{r}_1)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_5_{r}_0 z_8_{r}_0) z_8_{r}_1)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat x_5_{r}_0 z_7_{r}_0) z_7_{r}_1)) #b1))\n")

        # Permutation
        smtlib2_constr.append(f"(assert (= y_0_{r} z_6_{r}_0))\n")
        smtlib2_constr.append(f"(assert (= y_1_{r} z_7_{r}_1))\n")
        smtlib2_constr.append(f"(assert (= y_2_{r} z_8_{r}_1))\n")
        smtlib2_constr.append(f"(assert (= y_3_{r} z_9_{r}_1))\n")
        smtlib2_constr.append(f"(assert (= y_4_{r} z_10_{r}_1))\n")
        smtlib2_constr.append(f"(assert (= y_5_{r} z_11_{r}_5))\n")
        smtlib2_constr.append(f"(assert (= y_6_{r} x_0_{r}_0))\n")
        smtlib2_constr.append(f"(assert (= y_7_{r} x_1_{r}_0))\n")
        smtlib2_constr.append(f"(assert (= y_8_{r} x_2_{r}_0))\n")
        smtlib2_constr.append(f"(assert (= y_9_{r} x_3_{r}_0))\n")
        smtlib2_constr.append(f"(assert (= y_10_{r} x_4_{r}_0))\n")
        smtlib2_constr.append(f"(assert (= y_11_{r} x_5_{r}_0))\n\n")
    ...
    concat_terms = [f"x_{i}_0_0" for i in range(12)]
