...
def set_smtlib2(rounds):
    ...
    for r in range(rounds):
        ...
        # XOR
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_5_{r}_1 y_7_{r}_1) z_7_{r}_0)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_5_{r}_1 y_8_{r}_1) z_8_{r}_0)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_5_{r}_1 y_9_{r}_1) z_9_{r}_0)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_5_{r}_1 y_10_{r}_1) z_10_{r}_0)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_5_{r}_1 y_11_{r}_1) z_11_{r}_0)) #b1))\n")
        
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_4_{r}_1 z_11_{r}_0) z_11_{r}_1)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_3_{r}_1 z_11_{r}_1) z_11_{r}_2)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_2_{r}_1 z_11_{r}_2) z_11_{r}_3)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_1_{r}_1 z_11_{r}_3) z_11_{r}_4)) #b1))\n")

        for i in range(6):
            smtlib2_constr.append(f"(assert (= y_{i}_{r}_2 (ite (= y_{i}_{r}_1 #b00) #b00 #b11)))\n")
        smtlib2_constr.append("\n")

        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_0_{r}_2 z_11_{r}_4) z_11_{r}_5)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_1_{r}_2 z_10_{r}_0) z_10_{r}_1)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_2_{r}_2 z_9_{r}_0) z_9_{r}_1)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_3_{r}_2 z_8_{r}_0) z_8_{r}_1)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_4_{r}_2 z_7_{r}_0) z_7_{r}_1)) #b1))\n")
        smtlib2_constr.append(f"(assert (= (Table (concat (concat y_5_{r}_2 y_6_{r}_1) z_6_{r}_0)) #b1))\n")
